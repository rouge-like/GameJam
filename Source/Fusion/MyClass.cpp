...
void UMyClass::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (AudioCapture)
	{
...
void UMyClass::StopRecordingAndSave(const FString& FilePath)
{
	if (AudioCapture && AudioCapture->IsCapturingAudio())
	{
		AudioCapture->StopCapturingAudio();
...
void UMyClass::UploadWavFile(const FString& FilePath)
{
	...
	Request->OnProcessRequestComplete().BindUObject(this, &UMyClass::OnUploadCompleted);
	ActiveUploadRequest = Request;
	Request->ProcessRequest();
}

void UMyClass::HandleCaptureBuffer(const float* AudioData, int32 NumSamples)
{
...
bool UMyClass::EnsureAudioCaptureInitialized()
{
	if (!AudioCapture)
	{
...
bool UMyClass::WriteWavFile(const FString& FilePath, const TArray<int16>& InPCMData, int32 SampleRate, int32 NumChannels) const
{
...
void UMyClass::OnUploadCompleted(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
	ActiveUploadRequest.Reset();
